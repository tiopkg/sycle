...
Registry.prototype.define = function (/*name, desc, setup*/) {
    ...
    if (desc.base) {
        ...
        if (!def) {
            throw new Error(util.format('Unknown base model "%s" when define model "%s"', desc.base, name));
        }
    } else {
    ...
    if (def) desc = _.merge(_.cloneDeep(def), desc);
...
var DEF_KEYS = ['name', 'base', 'properties', 'fields', 'relations', 'settings'];
...
Registry.prototype.build = function (opts) {
    ...
    _.forEach(defs, function (v, k) {
        ...
        schemas.push(schema);
        if (opt.debug) {
            schema.on('log', function (sql, time) {
                console.log(sql);
            });
        }
    });
