...
module.exports = function(sycle) {
     ...
     */
    sycle.getCurrentContext = function() {
        // A placeholder method, see sycle.createContext() for the real version
        return null;
    };

    /**
     * Run the given function in such way that
     * `sycle.getCurrentContext` returns the
     ...
     */
    sycle.runInContext = function(fn, context) {
        ...
        var ns = context || sycle.createContext('sycle');
    ...
    /**
     * Create a new SycleContext instance that can be used
     * for `sycle.runInContext`.
     *
     * **NOTES**
     *
     * At the moment, `sycle.getCurrentContext` supports
     ...
     */
    sycle.createContext = function(scopeName) {
        ...
        if (!ns) {
            ...
            // Set up sycle.getCurrentContext()
            sycle.getCurrentContext = function() {
    ...
    function chain(child) {
        if (typeof child.getCurrentContext === 'function') {
            var childContext = new ChainedContext(child.getCurrentContext(),
                sycle.getCurrentContext());
            child.getCurrentContext = function() {
                return childContext;
            };
        } else {
            child.getCurrentContext = sycle.getCurrentContext;
        }
    }
};
